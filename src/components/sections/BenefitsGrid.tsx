...
export default function BenefitsGrid() {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div ref={contentRef} className="s-container relative z-10 w-full flex flex-col justify-around py-16 lg:py-16">
        {/* Headline */}
        <div className="text-center lg:text-left mb-8 lg:mb-16">
          ...
          <h2 className="s-h1 !text-white !text-[clamp(1.8rem,5vw,3.5rem)] lg:!text-6xl !leading-[0.85] !tracking-tighter uppercase lg:whitespace-nowrap">
            Engineered <br />
        ...
        {/* Spec Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 lg:gap-x-16 lg:gap-y-12 pb-12 lg:pb-0">
          {engineeringSpecs.map((spec) => (
            <div key={spec.id} className="group transition-all duration-700">
              <div className="flex items-center gap-6 lg:gap-8 min-h-[70px]">
                <div className="w-14 h-14 lg:w-14 lg:h-14 flex-shrink-0 flex items-center justify-center bg-zinc-950 border border-white/10 group-hover:border-primary/60 group-hover:bg-primary/5 transition-all duration-500 shadow-2xl">
                  <spec.icon size={26} className="text-zinc-500 group-hover:text-primary transition-colors" strokeWidth={1} />
                </div>
                <div className="space-y-1.5 overflow-hidden">
                  <div className="s-label !text-primary !text-[10px] tracking-[0.3em] font-black uppercase">
                    {spec.label}
                  </div>
                  <h3 className="s-h3 !text-white !text-2xl lg:!text-2xl !tracking-tighter group-hover:!text-primary transition-colors leading-none truncate">
                    {spec.title}
                  </h3>
                  <div className="text-zinc-400 group-hover:text-white transition-colors text-[11px] lg:text-[12px] uppercase font-bold tracking-tight">
                    {spec.value}
                  </div>
                </div>
              </div>
